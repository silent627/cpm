...
public class HouseholdMemberController {
     ...
     * 获取所有户籍成员列表（用于索引同步）
     */
    @ApiOperation(value = "获取所有户籍成员列表", notes = "获取所有户籍成员信息，用于搜索服务索引同步")
    @GetMapping("/all")
    public Result<List<HouseholdMember>> getAllHouseholdMembers() {
        List<HouseholdMember> members = householdMemberService.list();
        return Result.success(members);
    }

    /**
     * 查询户籍的所有成员（包含居民详细信息，使用Elasticsearch全文检索）
     */
    @ApiOperation(value = "查询户籍的所有成员", notes = "使用Elasticsearch全文检索分页查询指定户籍的所有成员，包含居民详细信息")
    ...
    public Result<Page<HouseholdMemberDTO>> getMembersByHouseholdId(
        ...
        try {
            ...
            Result<Map<String, Object>> searchResult = searchServiceClient.searchHouseholdMember("*", current - 1, size, householdId);
            
            if (searchResult == null || searchResult.getCode() != 200 || searchResult.getData() == null) {
                // 如果搜索服务失败，返回空结果
                Page<HouseholdMemberDTO> page = new Page<>(current, size);
                page.setTotal(0);
            ...
            // 转换为HouseholdMember实体列表
            List<HouseholdMember> members = new ArrayList<>();
            if (hits != null) {
                for (Map<String, Object> hit : hits) {
                    try {
                        HouseholdMember member = convertMapToHouseholdMember(hit);
                        members.add(member);
                    } catch (Exception e) {
                        // 忽略转换失败的数据
                        continue;
            ...
            // 构建返回结果
            Page<HouseholdMemberDTO> result = new Page<>(current, size);
    ...
    /**
     * 将Map转换为HouseholdMember对象
     */
    private HouseholdMember convertMapToHouseholdMember(Map<String, Object> map) {
        HouseholdMember member = new HouseholdMember();
        if (map.get("id") != null) {
            member.setId(Long.valueOf(map.get("id").toString()));
        }
        if (map.get("householdId") != null) {
            member.setHouseholdId(Long.valueOf(map.get("householdId").toString()));
        }
        if (map.get("residentId") != null) {
            member.setResidentId(Long.valueOf(map.get("residentId").toString()));
        }
        member.setRelationship((String) map.get("relationship"));
        if (map.get("createTime") != null) {
            try {
                member.setCreateTime(java.time.LocalDateTime.parse(map.get("createTime").toString()));
            } catch (Exception e) {
                // 忽略日期解析错误
            }
        }
        if (map.get("updateTime") != null) {
            try {
                member.setUpdateTime(java.time.LocalDateTime.parse(map.get("updateTime").toString()));
            } catch (Exception e) {
                // 忽略日期解析错误
            }
        }
